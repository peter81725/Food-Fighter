...
def Load_Yolo_model():
    ...
    if YOLO_FRAMEWORK == "tf": # TensorFlow detection
        ...
        if YOLO_CUSTOM_WEIGHTS == False:
            ...
            checkpoint = os.path.join(mod_parent, "checkpoints", TRAIN_MODEL_NAME)
            if TRAIN_YOLO_TINY:
                checkpoint += "_Tiny"
...
def draw_bbox(image, bboxes, CLASSES=YOLO_COCO_CLASSES, show_label=True, show_confidence = True, Text_colors=(255,255,0), rectangle_colors='', tracking=False):
    ...
    for i, bbox in enumerate(bboxes):
        ...
        obj_list.append((x1, y1))
        obj_list.append((x2, y2))
        bbox_list.append(obj_list)
...
def detect_image(Yolo, image_path, output_path, input_size=416, show=False, CLASSES=YOLO_COCO_CLASSES, score_threshold=0.3, iou_threshold=0.45, rectangle_colors=''):
    ...
    print(bboxes)
    image,cord = draw_bbox(original_image, bboxes, CLASSES=CLASSES, rectangle_colors=rectangle_colors)
    ...
    return original_image, image, cord
